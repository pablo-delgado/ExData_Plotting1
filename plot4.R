...
plot4 <- function() {
    ...
    ##
    # NOTE: The plot inner functions will access the data from the current scope.
    #       To ensure they work they just have to be called after get_cleandata()
    ##
    plot_1 <- function() {
    ...
    plot_2 <- function() {
        #x <- mutate(data, as.numeric(data$Voltage) * 0.15) %>% na.omit()
        x <- data
    ...
    plot_3 <- function() {
        ...
        # Add a legend
        legend("topright", bty = "n", col=c("black", "red", "blue"), c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), lty=1)
    }
    ...
    # Set up the graphic grid using par.
    par(mfrow=c(2,2))
